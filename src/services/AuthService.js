...
class AuthService {
  ...
  async getBreezeLoginUrl() {
    const r = await fetch('https://goodbuck-algo.onrender.com/api/breeze/login-url');
  ...
  async generateBreezeSession(apiSession) {
    const response = await fetch('https://goodbuck-algo.onrender.com/api/breeze/generate_session', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },

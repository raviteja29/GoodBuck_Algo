...
const DashboardGrid = ({ activeSection, dashboardData, userInfo, onDataRefresh }) => {
  ...
  const [pnlLoading, setPnlLoading] = useState(false); // Separate loading for P&L only
  const [lastRefresh, setLastRefresh] = useState(new Date());
  const [lastPnlRefresh, setLastPnlRefresh] = useState(new Date());
  // Removed pnlTicker - using only real P&L data

  // Fetch initial data ONCE on component mount (NO automatic refresh)
  useEffect(() => {
    ...
    // Only fetch once on mount - NO automatic refresh intervals
    fetchInitialData();
  }, []);

  // SEPARATE: P&L-only data refresh (no table re-renders)
  useEffect(() => {
    const updatePnLOnly = async () => {
      try {
        setPnlLoading(true);
        const positionsData = await TradingService.getPositions();
        
        // Update ONLY P&L calculations without causing table re-renders
        if (positionsData?.net) {
          // Update positions state only if P&L values actually changed
          setPositions(prevPositions => {
            const newPositions = positionsData.net;
            
            // Check if P&L data actually changed to avoid unnecessary re-renders
            const pnlChanged = newPositions.some((newPos, index) => {
              const prevPos = prevPositions[index];
              return !prevPos || newPos.pnl !== prevPos.pnl || newPos.day_pnl !== prevPos.day_pnl;
            });
            
            // Only update if P&L actually changed
            return pnlChanged ? newPositions : prevPositions;
          });
        }
    ...
    // Start P&L-only updates every 2 seconds (optimized for algo trading)
    const pnlInterval = setInterval(updatePnLOnly, 2000);

    return () => clearInterval(pnlInterval);
  }, []);

  // Manual refresh function (only this shows loading spinner)
  ...
  useEffect(() => {
    const unsubscribe = TradingService.subscribeToTicks(ticks => {
      ...
      // Update positions with new prices for real-time P&L
      setPositions(prevPositions => 
        prevPositions.map(position => {
          const tickUpdate = ticks.find(tick => tick.instrument_token === position.instrument_token);
          if (tickUpdate) {
            const newPnl = (tickUpdate.last_price - position.average_price) * position.quantity;
            return {
              ...position,
              last_price: tickUpdate.last_price,
              pnl: newPnl,
              // Update day P&L if we have opening price
              day_pnl: tickUpdate.ohlc?.open ? 
                (tickUpdate.last_price - tickUpdate.ohlc.open) * position.quantity : 
                position.day_pnl
            };
          }
          return position;
  ...
  // Calculate REAL P&L from actual positions (no mock data)
  const totalPnL = positions.reduce((sum, pos) => sum + (pos.pnl || 0), 0);
  const todayPnL = positions.reduce((sum, pos) => sum + (pos.day_pnl || 0), 0);
  
  // Use ONLY real P&L values from your live trading account
  const displayTotalPnL = totalPnL;
  const displayTodayPnL = todayPnL;
  
  // Calculate P&L trend (simple percentage change)
  const totalPnLTrend = displayTotalPnL !== 0 ? (displayTotalPnL / Math.abs(displayTotalPnL)) * 100 : 0;
  const todayPnLTrend = displayTodayPnL !== 0 ? (displayTodayPnL / Math.abs(displayTodayPnL)) * 100 : 0;
